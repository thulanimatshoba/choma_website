	...
	class MultiPostThumbnails {
		...
		public function register( $args = array() ) {
			...
			if ( null === $this->label || null === $this->id ) {
				if ( WP_DEBUG ) {
					trigger_error( sprintf( __( "The 'label' and 'id' values of the 'args' parameter of '%1\$s::%2\$s()' are required", 'multiple-post-thumbnails' ), __CLASS__, __FUNCTION__ ) );
				}
				return;
			}
		...
		public function enqueue_admin_scripts( $hook ) {
			global $wp_version;
			$template_url = plugins_url( 'stashed-out/admin/' );
